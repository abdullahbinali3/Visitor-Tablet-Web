<script>
  ...
  import mzone from "$vendor/timezone.js";
  ...
  $appData.loginError = false;
  $appData.initialUserLoggedInCheckDone = false;

  try {
    $appData.language = getLocaleFromNavigator().replace("_", "-").split("-")[0];
    if ($appData.language !== "en" && $appData.language !== "es") {
      $appData.language = "en";
    }
  } catch {
    $appData.language = "en";
  ...
  let noAccessErrorMessage;

  $appData.authToken = localStorage.getItem("authToken");
  let language = localStorage.getItem("language");
  if (language && language !== "null") {
    $appData.language = localStorage.getItem("language");
  }
...
{#if $isLoading}
  <Preloader />
{:else if $appData.initialUserLoggedInCheckDone && ($appData.noPortalAccess || ($user.userData && $user.userData.extendedData && $user.userData.extendedData.organizations && $user.userData.extendedData.organizations.length == 0))}
  <NoPortalAccess errorMessage={noAccessErrorMessage} />
  <!-- {:else if $appData.initialUserLoggedInCheckDone && $appData.loginError}
  <AppInitError
    retryFunc={$appData.authToken ? getUserData : null}
    errorMessage={$appData.loginError} /> -->
{:else if $appData.initialUserLoggedInCheckDone}
  <Router {routes} />
{:else}
  <Preloader />
{/if}
