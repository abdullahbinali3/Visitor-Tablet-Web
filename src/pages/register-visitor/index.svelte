  ...
  >
    ...
    <form on:submit={handleSubmit}>
      <div class="mb-6">
        ...
        {#if errors.host}<p class="mt-1 capitalize text-red-600 text-sm">
            {errors.host}
          </p>{/if}
      </div>
      ...
      <div class="mb-4">
        <InputText
          ...
          enterSubmit={true}
        />
        {#if errors.companyName}<p class="mt-1 capitalize text-red-600 text-sm">
            {errors.companyName}
          </p>{/if}
      </div>
      <div class="mb-4">
        <label for="purpose" class="block mb-2 text-gray-600"
          >{$t("registervisitor.purposePlaceholder")}</label
        >
        <textarea
        ...
        ></textarea>
        {#if errors.purpose}<p class="mt-1 capitalize text-red-600 text-sm">
            {errors.purpose}
          </p>{/if}
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          <DateInput
            label={$t("startDate")}
            bindValue={visitors.startDate}
            valueChanged={(newDate) => (visitors.startDate = newDate)}
          />
          {#if errors.startDate}<p class="text-red-600 capitalize text-sm mb-4">
              {errors.startDate}
            </p>{/if}
        </div>
        <div>
          <TimeInput
            label={$t("startTime")}
            bindValue={visitors.startTime}
            valueChanged={(newTime) => (visitors.startTime = newTime)}
          />
          {#if errors.startTime}<p class="text-red-600 capitalize text-sm mb-4">
              {errors.startTime}
            </p>{/if}
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          <DateInput
            label={$t("endDate")}
            bindValue={visitors.endDate}
            valueChanged={(newDate) => (visitors.endDate = newDate)}
          />
          {#if errors.endDate}<p class="text-red-600 capitalize text-sm mb-4">
              {errors.endDate}
            </p>{/if}
        </div>
        <div>
          <TimeInput
            label={$t("endTime")}
            bindValue={visitors.endTime}
            valueChanged={(newTime) => (visitors.endTime = newTime)}
          />
          {#if errors.endTime}<p class="text-red-600 capitalize text-sm mb-4">
              {errors.endTime}
            </p>{/if}
        </div>
      </div>
      {#if errors.dates}<p class="text-red-600 capitalize text-sm mb-4">
          {errors.dates}
        </p>{/if}
      ...
      {#if userLength?.length > 0}
        <h2 class="text-2xl mb-3 font-semibold">
          {$t("registervisitor.addedVisitors")}
        </h2>
        <div class=" mb-6">
        ...
        {#each [{ key: "firstName", placeholder: $t("registervisitor.firstnamePlaceholder") }, { key: "surname", placeholder: $t("registervisitor.surnamePlaceholder") }, { key: "email", type: "email", placeholder: $t("login.emailPlaceholder") }, { key: "mobileNumber", type: "number", placeholder: $t("registervisitor.mobilenumberPlaceholder") }] as { key, type = 'text', placeholder }}
          <div>
            <InputText
              ...
              on:input={() => (errors[key] = "")}
            />
            {#if errors[key]}<p class="mt-1 capitalize text-red-600 text-sm">
                {errors[key]}
              </p>{/if}
          </div>
        {/each}
      ...
      <div class="flex justify-center mb-4">
        <Button
          ...
          on:click={addVisitor}>{$t("AddVisitor")}</Button
        >
      </div>
